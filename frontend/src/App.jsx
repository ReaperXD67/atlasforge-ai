...
function RemotionLab({ form, setForm, system, startGeneration, submitting, activeJob, outputUrl, setToast }) {
  ...
  return <main className="remotion-lab">
    <section className="remotion-preview panel-surface">
      <div className="lab-heading"><div><span className="eyebrow">Remotion motion lab</span><h1>Music-led faceless films</h1><p>The live composition reacts to your BPM. The final cut searches locally ranked real race footage first; any synthetic insert must be explicitly necessary and pass admission.</p></div><span className="remotion-badge">FRAME-DETERMINISTIC</span></div>
    ...
    <form className="music-console panel-surface" onSubmit={submit}>
      ...
      <div className="toggle-row lab-toggles"><button type="button" className={form.local_ai ? "active" : ""} onClick={() => setForm({ ...form, local_ai: !form.local_ai })}><Sparkle weight="fill" /> Strict AI fallback <span>{form.local_ai ? "Armed" : "Off"}</span></button><button type="button" className={form.stock_images ? "active" : ""} onClick={() => setForm({ ...form, stock_images: !form.stock_images })}><FilmReel /> Real footage first <span>{form.stock_images ? "On" : "Off"}</span></button></div>
      <p className="hardware-note"><StatusDot ok={system.nvenc} /> Beat analysis and ranking stay local. Arming AI does not create a shot by itself; ordinary racing scenes remain real-footage-first. Final audio is your untouched master encoded at 320 kbps.</p>
